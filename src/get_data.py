...
def step1(num):
    ...
    # select chooses display all pages/players' records; select2 chooses season; select3 chooses season type
    ...
    header = [a for a in header if not 'RANK' in a][1:]
    #each row contains stats of a player and all his stats
    rows = table.findAll('tr')[1:]
    #scraping out all player's stats to a 2d array
    player_stat = [[td.getText().strip() for td in rows[i].findAll('td')[1:]] for i in range(len(rows))]
...
def step2():
    ...
    df_salary23.to_csv("salary_2023_2024") #export season 23/24
...
def step3(num):
    ...
    player_salary = [[td.getText(strip=True)for td in row[i].findAll('td')[1::2]]for i in range(len(row))]
...
def select2(driver,num):
    select2 = Select(driver.find_element(By.XPATH, r"/html/body/div[1]/div[2]/div[2]/div[3]/section[1]/div/div/div[1]/label/div/select"))
    select2.select_by_index((2023-num)) # choose season (2023-num)=index 
def select3(driver):
    select3 = Select(driver.find_element(By.XPATH, r"/html/body/div[1]/div[2]/div[2]/div[3]/section[1]/div/div/div[2]/label/div/select"))
