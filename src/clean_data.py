...
def dfmerger(start):
    ...
    df_salary = df_salary.rename(
        columns={df_salary.columns[0]: "Player",
                 df_salary.columns[1]: "salary"})
    ...
    merged_df = pd.merge(df_salary, df_stats, on=[
                         'first_name', 'last_name'], how='inner')
    # re-format salary from string to float
    merged_df['salary'] = merged_df['salary'].str.replace(
